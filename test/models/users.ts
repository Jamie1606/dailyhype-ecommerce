...
import { ICheckLogin } from "../types/user";
...
//   return pool.query(sql, [email]).then(function (result: QueryResult<ICheckLogin>) {
//     const rows = result.rows;

//     if (rows.length === 0) {
//       // throw new EMPTY_RESULT_ERROR(`Email ${email} not found!`);
//     }

//     const retrievedPasswordHash = rows[0].password;

//     return bcrypt.compare(password, retrievedPasswordHash).then(function (isMatch) {
//       const isVerified = rows[0].verified_email;
//       if (!isMatch) {
...
//     return pool
//       .query(sql, [hashedPassword, userID])
//       .then((result) => {
...
// export function signup(name: string, email: string, password: string, phone: number, gender: string) {
...
//     return pool
//       .query(checkRoleQuery, [defaultRole])
//       .then((result) => {
//         let roleId;
...
//           return pool.query(sql, [name, email, hashedPassword, phone, gender, roleId, defaultStatus, method]).catch(function (error) {
//             throw error;
//           });
//         } else {
...
// export function saveVerificationCode(code: string, email: string) {
//   const sql = "UPDATE appuser SET code = $1, createdcodeat = NOW() WHERE email = $2 AND method = 'normal' RETURNING *";
//   return pool.query(sql, [code, email]).catch(function (error) {
//     throw error;
//   });
// }

// export function verification(code: string, email: string) {
...
// `;
//   return pool.query(sql, [email]).then(function (result) {
//     if (result.rows.length === 0) {
//       throw new Error("User not found");
//     }
...
//       return pool.query(updateSql, [email])
//         .then(function () {
//           return result.rows[0];
//         })
